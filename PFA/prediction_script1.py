...
def register_device_connection():
    """S'identifier auprès du serveur Node.js"""
    try:
        device_payload = {
            "device_id": MACHINE_ID,
            "type": "raspberry_pi",
        ...
        print(f"📱 Enregistrement du device {MACHINE_ID} auprès du serveur...")
        response = requests.post(f"{BACKEND_SERVER_URL}/api/raspberry-pi/connect", 
                               json=device_payload, timeout=10)
        
        if response.status_code == 200:
            result = response.json()
            print(f"✅ Device enregistré avec succès: {result}")
            return True
        else:
            print(f"⚠️ Erreur enregistrement HTTP {response.status_code}: {response.text}")
            return False
...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        ...
        # S'enregistrer via HTTP ET via MQTT
        http_success = register_device_connection()
        
        # Envoyer le statut via MQTT aussi
        send_device_status()
        
        if http_success:
            print(f"✅ Device {MACHINE_ID} complètement enregistré")
        else:
            print(f"⚠️ Enregistrement partiel - MQTT OK, HTTP échoué")
...
def main():
    ...
    # S'enregistrer immédiatement si possible (non-bloquant)
    print("📱 Tentative d'enregistrement préalable...")
